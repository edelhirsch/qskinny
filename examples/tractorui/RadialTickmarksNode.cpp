...
void RadialTickmarksNode::update( const QColor& color, const QRectF& rect,
    const QskArcMetrics& arcMetrics, const QskTickmarks& tickmarks,
    const QskIntervalF& interval, int lineWidth )
{
    ...
    if( ( hash != m_hash ) || ( rect != m_rect ) )
    {
        ...
        const auto spanAngle = arcMetrics.spanAngle();
        ...
        for( int i = TM::MinorTick; i <= TM::MajorTick; i++ )
        {
            ...
            const auto factor = i == TM::MajorTick ? 1 : 0.6;
            const auto needleRadius = radius - factor * arcMetrics.thickness();
